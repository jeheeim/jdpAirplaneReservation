...
namespace HashTagUI
{
	...
	public class ClientSocket
	{
		...
		public static readonly string prePath = Directory.GetParent(Application.StartupPath).ToString();
		...
		private void makeclientSocketAccount()
		{
			string line;

			StreamReader file = new StreamReader(prePath + "\\Data\\account.txt");

			while ((line = file.ReadLine()) != null)
			{
				Account temp = new Account(line);
		...
		{
			StreamWriter file = new StreamWriter(prePath + "\\Data\\Airplane.txt");
		...
		public bool ModifyInfo(Account account)
		{
			try
			{
				dic_userInfo[account.id].name = account.name;
